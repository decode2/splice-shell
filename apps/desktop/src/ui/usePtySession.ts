...
// Inputs the PTY session state machine needs from TerminalView. These mirror the
// component's session-relevant props one-to-one; `visible` stays in the view
// because it is a pure CSS concern with no bearing on the session lifecycle.
export type UsePtySessionOptions = {
  active: boolean;
...
// handling, and disposal ordering. TerminalView is the presentational shell that
// mounts the returned host ref and reacts to `active`/`visible`; every session
// behavior lives here so the mount effect can keep `[]` deps and read live
// values through refs without ever remounting xterm on a parent re-render.
